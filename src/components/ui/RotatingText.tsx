      ...
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        ...
        >
          {elements.map((wordObj, wordIndex, array) => {
            const previousCharsCount = array.slice(0, wordIndex).reduce((sum, word) => sum + (word.characters as any[]).length, 0);
            return (
              <span key={wordIndex} className={cn('text-rotate-word', splitLevelClassName)}>
                {(wordObj.characters as any[]).map((char, charIndex) => (
                  <motion.span
                    key={charIndex}
                    initial={initial}
                    animate={animate}
                    ...
                    transition={{
                      ...
                      delay: getStaggerDelay(
                        ...
                        array.reduce((sum, word) => sum + (word.characters as any[]).length, 0)
                      )
                    }}
